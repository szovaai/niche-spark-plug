...
// Generate email sequence export in multiple formats (with sanitization)
const generateEmailSequenceExport = (sequence: EmailSequence14Day): { plainText: string; csv: string; html: string } => {
  // Plain Text Format - sanitize all content
...
`);
  ...
  // CSV Format - sanitize content before escaping
  ...
  const csvRows = sequence.emails.map(email => {
    const escapeCsv = (str: string) => `"${sanitizeText(str || '').replace(/"/g, '""').replace(/\n/g, ' ')}"`;
    return [
      email.day,
